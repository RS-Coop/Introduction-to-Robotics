...
def do_color_filtering(img):
    ...
    # DONE: Iterate through each pixel (x,y) coordinate of the image,
...
def expand_nr(img_mask, cur_coord, coordinates_in_blob):
    ...
    while len(coordinate_list) > 0:
        ...
        # DONE: Check to make sure cur_coordinate is in bounds, otherwise 'continue'
        if cur_coordinate[0] < 0 or cur_coordinate[1] < 0 or cur_coordinate[0] >= img_mask.shape[0] or cur_coordinate[1] >= img_mask.shape[1]:
            continue
        # DONE: Check to see if the value is 0, if so, 'continue'
        if img_mask[cur_coordinate[0], cur_coordinate[1]] == 0:
            continue

        # DONE: Set image mask at this coordinate to 0
        img_mask[cur_coordinate[0], cur_coordinate[1]] = 0
        # DONE: Add this coordinate to 'coordinates_in_blob'
        coordinates_in_blob.append(cur_coordinate)

        # DONE: Add all neighboring coordinates (above, below, left, right) to coordinate_list to expand to them
...
def get_blobs(img_mask):
    ...
    # DONE: Copy image mask into local variable using copy.copy
    mask_local = copy.copy(img_mask)
    blobs_list = [] # List of all blobs, each element being a list of coordinates belonging to each blob

    # DONE: Iterate through all 'y' coordinates in img_mask
    for y in range(img_mask_height):
    #   DONE: Iterate through all 'x' coordinates in img_mask
        for x in range(img_mask_width):
    #     DONE: If mask value at [y,x] is 1, call expand_nr on copy of image mask and coordinate (y,x), giving a third argument of an empty list to populate with blob_coords.
            if mask_local[y,x] == 1:
                blob = []
                #     DONE: Add blob_coords to blobs_list
                blob = expand_nr(mask_local, (y,x), blob)
